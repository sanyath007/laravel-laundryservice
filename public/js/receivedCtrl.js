app.controller('receivedCtrl', function($scope, $http, toaster, ModalService, CONFIG) {
/** ################################################################################## */
...
/** ################################################################################## */
	...
	$scope.calculateTotal = function () {
		...
		$("#tmpTotal").val($scope.sentoutTotalWeight)
		console.log($("#tmpTotal").val())
	}

	$scope.submitReceivedForm = function () {	
		console.log(event.target)
		console.log($("#sentout_date").val())

		if ($("#invoice").val() == '' || $("#total_weight").val() == 0) {
			event.preventDefault()
			toaster.pop('error', "", "กรุณากรอกข้อมูลเลขที่บิล หรือ น้ำหนักผ้าสะอาดก่อน !!!")
		} else if ($("#tmpTotal").val() == 0) {
			event.preventDefault()
			toaster.pop('error', "", "กรุณากรอกจำนวนผ้าแต่ละชนิดก่อน !!!")
		} else {
