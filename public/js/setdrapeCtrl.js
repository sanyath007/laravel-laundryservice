app.controller('setdrapeCtrl', function($scope, $http, toaster, ModalService, CONFIG) {
/** ################################################################################## */
...
/** ################################################################################## */
	...
	$scope.calculateTotalRequest = function () {
		console.log(event.target)
		$scope.totalRequest += ($("#"+event.target.id).val() == '') ? 0 : parseInt($("#"+event.target.id).val());
		$("#total_request").val($scope.totalRequest)
		console.log($("#total_request").val())
	}
	...
	$scope.loadSetforWard = function () {
		...
		$http.get(baseUrl + '/set/ajaxsetforstock/' + $('#_stock').val())
		.then(function (res) {
			console.log(res.data)
			$scope.setforward = res.data
		})
	}
